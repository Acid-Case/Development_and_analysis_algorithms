# Разработать алгоритм и программу внутренней сортировки значений в
# таблице (бинарные вставки).
...
class Matrix:
    ...
    def sort_matrix(self) -> None:
        flat_list = self.create_flat_list()
        for i in range(1, len(flat_list)):
            key = flat_list[i]
            right = i-1
            ind = self.__binary_search(flat_list, right, key)
            self.__shift_insert(flat_list, right, key, ind)
        self.__update_matrix(flat_list)

    def create_flat_list(self) -> list[int | float]:
        return [el for row in self.__matrix for el in row]

    def __binary_search(self, row: list[int | float], right: int, key: int | float) -> int:
        left = 0
        while left <= right:
            mid = (left + right) // 2
    ...
    def __shift_insert(self, row: list[int | float], right: int, key: int | float, ind: int) -> None:
        for i in range(right + 1, ind, -1):
            row[i] = row[i-1]
        row[ind] = key

    def __update_matrix(self, flat_list: list[int | float]) -> None:
        for i in range(self.__row):
            start = i * self.__col
            end = start + self.__col
            self.__matrix[i] = flat_list[start:end]
...
def main():
    ...
    print("Матрица после сортировки всех элементов:")
    print(matrix)
